...
class UnderlyingGenerator:
  ...
  def generate_path(self, initial_state = None, schedule = None ):
    ...
    if schedule is None: schedule = np.linspace(0, 1, 11)
  ...
  def generate_paths(self, initial_state = None, schedules = None, n = 3):

    # Setting defaults
    if schedules is None: schedules = [Schedule(np.linspace(0,1,11)) for i in range(0,n)]
